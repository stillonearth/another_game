    ...
    if let Some((deck_entity, _)) = q_decks.iter().find(|(_, deck)| deck.marker == 1) {
        ...
        *last_turn = phase_state.turn_number;
        ...
        commands.spawn_task(move || async move {
            ...
            AsyncWorld.write_message(DeckShuffle {
                deck_entity,
                duration: 50,
            })?;
            AsyncWorld.sleep(2.0).await;
            // Send draw event
            AsyncWorld.write_message(DrawToHand {
                deck_entity,
                num_cards: cards_to_draw,
                player: 1,
            })?;
            // Send phase event
            AsyncWorld.write_message(CardDrawnEvent {
    ...
    if let Some((deck_entity, _)) = q_decks.iter().find(|(_, deck)| deck.marker == 2) {
        ...
        *last_turn = phase_state.turn_number;
        ...
        commands.spawn_task(move || async move {
            ...
            // Send phase event
            AsyncWorld.write_message(AdversaryCardDrawnEvent {
                card_count: cards_to_draw,
            })?;
